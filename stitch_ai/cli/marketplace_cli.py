...
def add_marketplace_subparsers(subparsers, handlers):
    ...
    list_spaces_parser.add_argument('type', help='Type of memory space (e.g. public, private, etc)')
    list_spaces_parser.add_argument('--user-id', help='User ID (optional)', default=None)
    ...
    list_memory_parser.add_argument('user_id', help='User ID')
    list_memory_parser.add_argument('repository', help='Repository name')
    list_memory_parser.add_argument('api_key', help='API key')
    ...
    purchase_parser.add_argument('user_id', help='User ID')
    purchase_parser.add_argument('api_key', help='API key')
...
def handle_market_list_memory(sdk: StitchSDK, args: argparse.Namespace) -> None:
    try:
        ...
        response = sdk.marketplace.list_memory(args.user_id, args.repository, args.api_key, body)
...
def handle_market_purchase(sdk: StitchSDK, args: argparse.Namespace) -> None:
    try:
        print('_' * 50)
        body = json.loads(args.body)
        response = sdk.marketplace.purchase_memory(args.user_id, args.api_key, body)
